    ...
    for (const account of defaultAccounts) {
        if (account.children) {
            ...
            for (const child of account.children) {
                ...
                const { children: _, ...childData }: { children?: any[], [key: string]: any } = child;
...
export const addAccount = async (accountData: AccountFormData, parentId: string | null): Promise<{ success: boolean; message: string; accountId?: string }> => {
  ...
  if (parentId) {
      const parentDocRef = doc(db, 'accounts', parentId);
      const parentSnap = await getDoc(parentDocRef);
      if (parentSnap.exists()) {
          const parentCode = parentSnap.data().code;
          if (!accountData.code.startsWith(parentCode)) {
               return { success: false, message: `يجب أن يبدأ رمز الحساب الفرعي برمز الحساب الأصل (${parentCode}).` };
          }
      } else {
           return { success: false, message: "لم يتم العثور على الحساب الأصل." };
      }
  }

  const accountsCol = collection(db, 'accounts');
  // Destructure parentId out of accountData to avoid duplication in Firestore document
  const { parentId: formParentId, ...restOfData } = accountData; 
  const newAccountData: any = {
    ...restOfData,
    // Use the explicitly passed parentId, which is the source of truth
