...
const findFirstTransactionalAccount = (accounts: Account[], groupName: 'Revenues' | 'Expenses'): Account | null => {
    const root = accounts.find(acc => acc.group === groupName);
    if (!root) return null;

    const findRecursive = (account: Account): Account | null => {
        // A transactional account has no children.
...
const seedTransactions = async () => {
    ...
    const revenuesAccount = findFirstTransactionalAccount(allAccounts, 'Revenues');
    const expensesAccount = findFirstTransactionalAccount(allAccounts, 'Expenses');

    if (!revenuesAccount || !expensesAccount) {
        console.error("Could not find default transactional revenue/expense accounts for seeding transactions. Seeding skipped.");
        return;
    }
